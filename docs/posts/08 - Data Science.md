...
2. **🔵 Inverse Filter (The Correction):** We generate a curve that boosts +10dB at 50Hz.
...
Once we have these `taps`, we simply "convolve" them with the incoming audio stream. It’s a fast, vectorized operation that runs in real-time on a Raspberry Pi.
