"""
Tests for AudioAppConfig argument parsing and validation.
"""
...
from src.audio_app_config import AudioAppArgs
from src.settings import settings


@pytest.fixture
def mock_device_selector():
    """Mock the AudioDeviceSelector to prevent hardware calls."""
    with patch("src.audio_app_config.AudioDeviceSelector") as mock:
        # Setup a default dummy device
        device_instance = mock.return_value
        device_instance.id = 1
        device_instance.name = "Dummy Device"
        device_instance.native_rate = 48000
        ...
        device_instance.is_default = True
        yield mock


def test_validate_args_adjusts_buffer(mock_device_selector):
...
def test_validate_args_adjusts_buffer_rounding(mock_device_selector):
...
@patch("src.audio_app_config.AudioDeviceCalibrator")
def test_validate_args_with_calibration(mock_calibrator_cls, mock_device_selector):
    ...
    mock_device_selector.return_value.is_default = False
...
def test_missing_calibration_file_error(mock_device_selector):
    ...
    with pytest.raises(ValueError, match="Calibration file required"):
        AudioAppArgs.validate_args(args)
