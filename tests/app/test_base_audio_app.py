...
@patch("src.base_audio_app.AudioStreamsConsumerThread")
@patch("src.base_audio_app.AudioStreamsListenerThread")
def test_app_initialization_and_thread_setup(mock_listener_cls, mock_consumer_cls, mock_dependencies):
    """
    Test that BaseAudioApp initializes correctly and sets up the
    producer/consumer threads in its thread list.
    """
    audio_config, pipeline = mock_dependencies

    # Instantiate the app
    app = BaseAudioApp(audio_config, pipeline)
    ...
    # Trigger thread setup (normally called by BaseThreadApp.run/start,
    ...
    assert app._threads[0].name == "AudioListenerThread"
    assert app._threads[1].name == "AudioConsumerThread"
