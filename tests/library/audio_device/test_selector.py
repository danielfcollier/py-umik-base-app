"""
Unit tests for AudioDeviceSelector.
...
def mock_sounddevice():
    ...
    with patch("src.library.audio_device.selector.sd") as mock_sd:
...
def test_select_default_device(mock_sounddevice):
    ...
    selector = AudioDeviceSelector(target_id=None)
...
def test_select_specific_device(mock_sounddevice):
    ...
    selector = AudioDeviceSelector(target_id=1)
...
def test_device_not_found_exits(mock_sounddevice):
    ...
    with pytest.raises(SystemExit) as excinfo:
        AudioDeviceSelector(target_id=99)
...
def test_show_audio_devices(mock_sounddevice):
    ...
    with patch("src.library.audio_device.selector.logger") as mock_logger:
        AudioDeviceSelector.show_audio_devices(selected_id=1)
...
def test_unexpected_error_raises(mock_sounddevice):
    ...
    with pytest.raises(Exception, match="Boom"):
        AudioDeviceSelector(target_id=0)
