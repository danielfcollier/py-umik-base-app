"""
Unit tests for the AudioDeviceCalibrator class.
...
from src.library.cache_strategy import NoOpFilterCache
...
def mock_firwin2():
    ...
    """
    with patch("src.library.audio_device.calibrator.firwin2") as mock:
        # Create a mock filter array.
        # AudioDeviceCalibrator designs a filter of length `num_taps`.
...
def test_initialization_parses_file_and_designs_filter(mock_firwin2):
    ...
    with patch("builtins.open", mock_open(read_data=DUMMY_CAL_DATA)) as mock_file:
        # Initialize Calibrator with NoOp cache (crucial for isolation)
        calibrator = AudioDeviceCalibrator(
...
def test_get_sensitivity_values_parsing():
    ...
    with patch("builtins.open", mock_open(read_data=dummy_content)):
        ...
        sens_dbfs, ref_spl = AudioDeviceCalibrator.get_sensitivity_values("dummy.txt")
...
def test_get_sensitivity_values_missing_header():
    ...
    with patch("builtins.open", mock_open(read_data=dummy_content)):
        with pytest.raises(ValueError, match="not found"):
            AudioDeviceCalibrator.get_sensitivity_values("dummy.txt")
