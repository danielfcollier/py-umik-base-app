...
a shared queue for consumption by other threads.
...
class AudioStreamsListenerThread:
    ...
    using `sounddevice.InputStream` and place them onto a shared queue for
    further processing by other threads (Consumers).

    It runs until a `stop_event` is set, ensuring graceful shutdown. It uses
    high priority to minimize the risk of audio buffer overflows.
    """

    def __init__(
        self, audio_device_config: AudioDeviceConfig, shared_audio_queue: queue.Queue, stop_event: threading.Event
    ):
        """
        ...
        :param shared_audio_queue: A thread-safe `queue.Queue` instance. Raw audio
                                   chunks captured from the microphone will be put
                                   onto this queue.
    ...
    def run(self):
        ...
        `sounddevice.InputStream` and puts them onto the shared queue as tuples
        containing the audio data (numpy array) and a timestamp.

        This thread prioritizes reading from the audio buffer to prevent overflows
        and performs minimal processing. It runs until the `stop_event` is set.
        """
        ...
        try:
            ...
            with sd.InputStream(
                ...
                dtype=dtype,
                channels=1,
                high_priority=self._audio_device_config.high_priority,
            ) as stream:
                logger.debug(
