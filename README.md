...
2. **Batch Analysis**
Processes an entire folder of WAV files and aggregates the data into one single CSV.
- **Linux/Mac**:

```bash
make batch-analyze DIR="recordings/" CSV_OUT="report.csv" F="umik-1/700xxxx.txt"
```
...
- **View (Popup Window):**
```bash
make plot-view IN="recordings/test.wav_metrics.csv"

- **Save (To Image):**
```bash
make plot-save IN="recordings/test.wav_metrics.csv" PLOT_OUT="chart.png"
```
...
![Sample Recording Metrics](sample_recording_metrics.png)
